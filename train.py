...
def train(args):
    ...
    if args.deepspeed:
        ...
        if is_swapping_blocks:
            accelerator.unwrap_model(jointconn_model).move_to_device_except_swap_blocks(accelerator.device)  # reduce peak memory usage
    ...
    edge_energy = EdgeEnergyMap().to(accelerator.device) if args.enable_jointconn_v2 else None
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*training_models):
                ...
                if args.enable_jointconn_v2:
                    eps_t = 1e-5
                    rand_time = lambda: torch.rand(pair_bsz, device=accelerator.device, dtype=weight_dtype) * (1.0 - 2 * eps_t) + eps_t
                    is_joint = torch.rand(pair_bsz, device=accelerator.device) < args.p_joint_task
                    is_sync = torch.rand(pair_bsz, device=accelerator.device) < args.p_sync
                    ...
                    m_x = torch.ones(pair_bsz, device=accelerator.device, dtype=weight_dtype)
                    m_y = is_joint.to(weight_dtype)
                    lambda_x = torch.ones(pair_bsz, device=accelerator.device, dtype=weight_dtype)
                    ...
                    if not args.is_latent_training and "depth" in batch:
                        depth_1ch = ((batch["depth"].to(accelerator.device, dtype=weight_dtype) + 1.0) / 2.0).mean(dim=1, keepdim=True)
                        e_loss = edge_energy(depth_1ch, (packed_latent_height, packed_latent_width)).to(accelerator.device, dtype=weight_dtype)
                    elif "depth_edge" in batch:
                        e_loss = batch["depth_edge"].to(accelerator.device, dtype=weight_dtype)
                    else:
                        e_loss = torch.zeros(pair_bsz, packed_noisy_model_input.shape[1], device=accelerator.device, dtype=weight_dtype)
                    ...
                    """ Adopt the legacy paired RGB-depth timestep sampling strategy. """
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.save_every_n_steps == 0:
                    ...
                    json_path = os.path.join(
                        args.output_dir,
                        f"jointconn_v2_addons_step_{global_step:06d}.json"
                    )
