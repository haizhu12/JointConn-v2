...
class DoubleStreamBlock(nn.Module):
    ...
    def _forward(
        ...
        if hasattr(self, "jointconn_v2") and token_hw is not None:
            pe_img = pe[:, :, txt.shape[1] :]
            img_q_rope, img_k_rope = apply_rope(img_q, img_k, pe_img)
            img_attn = img_attn + self.jointconn_v2.forward_from_qkv(
                h=img,
                q=img_q_rope,
                k=img_k_rope,
                v=img_v,
            ...
            txt_o, img_o = o_attn[:, : txt.shape[1]], o_attn[:, txt.shape[1] :]
            txt_attn = txt_attn + txt_o
            img_attn = img_attn + img_o
    ...
    def forward(
        ...
        if self.training and self.gradient_checkpointing:
            ...
            return self._forward(img, txt, vec, pe, txt_attention_mask, blend_source, e_att, token_hw, task_masks, layer_index)
...
class SingleStreamBlock(nn.Module):
    ...
    def _forward(
        ...
        if hasattr(self, "jointconn_v2") and token_hw is not None and txt_len is not None:
            q_img, k_img, v_img = q[:, :, txt_len:], k[:, :, txt_len:], v[:, :, txt_len:]
            pe_img = pe[:, :, txt_len:]
            q_img, k_img = apply_rope(q_img, k_img, pe_img)
            img_res = self.jointconn_v2.forward_from_qkv(
                h=x[:, txt_len:],
                q=q_img,
                k=k_img,
                v=v_img,
            ...
            attn = torch.cat([attn[:, :txt_len], attn[:, txt_len:] + img_res], dim=1)
        else:
    ...
    def forward(
        ...
        if self.training and self.gradient_checkpointing:
            ...
            return self._forward(x, vec, pe, txt_attention_mask, blend_source, txt_len, e_att, token_hw, task_masks, layer_index)
...
class JointConnV2Model(nn.Module):
    """
    ...
    def wait_for_pending_block_swaps(self):
        if self.blocks_to_swap is None or self.blocks_to_swap == 0:
            return
    ...
    def forward(
        ...
        e_att: Tensor | None = None,
        task_masks: dict | None = None,
        ...
        self.last_jointconn_stats = []

        token_hw = None
        if img_ids is not None and img_ids.ndim == 3 and img_ids.shape[1] == img.shape[1]:
        ...
        if not self.blocks_to_swap:
            for block_idx, block in enumerate(self.double_blocks):
                ...
                if hasattr(block, "jointconn_v2") and block.jointconn_v2.last_stats is not None:
                    self.last_jointconn_stats.append(block.jointconn_v2.last_stats)
            ...
            for block_idx, block in enumerate(self.single_blocks):
                img = block(
                    ...
                    token_hw=token_hw,
                    task_masks=task_masks,
                    layer_index=block_idx + len(self.double_blocks),
                )
                if hasattr(block, "jointconn_v2") and block.jointconn_v2.last_stats is not None:
                    self.last_jointconn_stats.append(block.jointconn_v2.last_stats)
        ...
        if return_jointconn_stats:
            return img, self.last_jointconn_stats
        return img
