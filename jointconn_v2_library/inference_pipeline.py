...
from jointconn_v2_library.geometry import zero_edge
...
def joint_generation(
    ...
    jointconn_model: jointconn_v2_model.JointConnV2Model,
    ...
    sample_steps = getattr(args, "sample_steps", 20)
    width = resolution[1]
    height = resolution[0]
    ...
    if getattr(args, "enable_jointconn_v2", False):
        e_att = zero_edge(pair_batch, noise.shape[1], accelerator.device, weight_dtype)
        task_masks = {
            "m_x": torch.ones(pair_batch, device=accelerator.device, dtype=weight_dtype),
            "m_y": torch.ones(pair_batch, device=accelerator.device, dtype=weight_dtype),
            "lambda_x": torch.ones(pair_batch, device=accelerator.device, dtype=weight_dtype),
            "lambda_y": torch.ones(pair_batch, device=accelerator.device, dtype=weight_dtype),
        }

    # denoising
    with accelerator.autocast(), torch.no_grad():
        x = denoise(jointconn_model, noise, img_ids, t5_out, txt_ids, l_pooled, args, timesteps=timesteps, guidance=scale, t5_attn_mask=t5_attn_mask, controlnet=controlnet, controlnet_img=controlnet_image, e_att=e_att, task_masks=task_masks)
...
def denoise(
    model: jointconn_v2_model.JointConnV2Model,
    img: torch.Tensor,
    ...
    e_att: Optional[torch.Tensor] = None,
    task_masks: Optional[dict] = None,
):
...
def conditional_generation(
    ...
    # denoising
    with accelerator.autocast(), torch.no_grad():
        x = conditional_denoise(args, gen_type, jointconn_model, noise, img_ids, t5_out, txt_ids, l_pooled, condition_latent,  timesteps=timesteps, guidance=scale, t5_attn_mask=t5_attn_mask, controlnet=controlnet, controlnet_img=controlnet_image, condition_e_att=condition_e_att)
...
def conditional_denoise(
    ...
    controlnet: Optional[flux_models.ControlNetFlux] = None,
    controlnet_img: Optional[torch.Tensor] = None,
    condition_e_att: Optional[torch.Tensor] = None,
):  
    ...
    if getattr(args, "enable_jointconn_v2", False):
        if gen_type != "depth_to_image":
            raise ValueError("JointConn-v2 conditional inference currently supports depth_to_image only.")
        pair_batch = 1
        if condition_e_att is not None:
            e_att = condition_e_att.to(img.device, dtype=img.dtype)
        else:
            e_att = zero_edge(pair_batch, img.shape[1], img.device, img.dtype)
        task_masks = {
            "m_x": torch.ones(pair_batch, device=img.device, dtype=img.dtype),
            "m_y": torch.zeros(pair_batch, device=img.device, dtype=img.dtype),
            "lambda_x": torch.ones(pair_batch, device=img.device, dtype=img.dtype),
            "lambda_y": torch.zeros(pair_batch, device=img.device, dtype=img.dtype),
        }
...
def sample_images(
    ...
    It loads pre-computed text latents and runs inference using the JointConn-v2 model.
    """
    ...
    flux = accelerator.unwrap_model(jointconn_model)
        ...
        with torch.no_grad(), accelerator.autocast():
            for prompt_latent in prompt_latents:
                sample_image_inference(
                    accelerator, args, jointconn_model, text_encoders, ae, save_dir,
...
def sample_image_inference(
    ...
    # Load latents
    l_pooled = torch.load(prompt_latent[0], map_location='cpu').to(weight_dtype).to(jointconn_model.device).unsqueeze(0).repeat(2, 1)
    t5_out = torch.load(prompt_latent[1], map_location='cpu').to(weight_dtype).to(jointconn_model.device).unsqueeze(0).repeat(2, 1, 1)

    if args.is_txt_ids_training:
        txt_ids = torch.load(prompt_latent[2], map_location='cpu').to(weight_dtype).to(jointconn_model.device).unsqueeze(0).repeat(2, 1, 1)
    else:
        txt_ids = torch.zeros(t5_out.shape[0], t5_out.shape[1], 3, device=t5_out.device)

    if args.is_attnmask_training:
        t5_attn_mask = torch.load(prompt_latent[3], map_location='cpu').to(weight_dtype).to(jointconn_model.device).unsqueeze(0).repeat(2, 1)
    else:
        t5_attn_mask = None
    ...
    # Denoising
    with accelerator.autocast(), torch.no_grad():
        x = denoise(jointconn_model, noise, img_ids, t5_out, txt_ids, l_pooled, args, timesteps=timesteps, guidance=scale, t5_attn_mask=t5_attn_mask, controlnet=controlnet, controlnet_img=controlnet_image)
