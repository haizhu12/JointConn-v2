...
class JointDataset(data.Dataset):
    def __init__(self, args, image_folder, drop_text, depth_transform):
        ...
        self.empty_prompt_dir = "empty_prompts"
        self.has_empty_prompt = all(
            os.path.isfile(os.path.join(self.empty_prompt_dir, subdir, "empty.pt"))
            for subdir in ("clip_latents", "t5_latents")
        )
    ...
    def __getitem__(self, index):
        ...
        if self.args.is_latent_training:
            use_empty_text = self.has_empty_prompt and np.random.rand() < self.drop_text

            if use_empty_text:
                base_path = self.empty_prompt_dir
                sample = {
